# What covered in notes
-  **Hooks**
-  **Serial execution**
-  **Parameterization**, 

---

## 1️⃣ Global Hooks (beforeAll, beforeEach, afterEach, afterAll)

### 🔹 Code Example
```ts
import { test, expect } from '@playwright/test';

test.beforeAll(async () => {
  console.log('called before all');
});

test.beforeEach(async () => {
  console.log('before each');
});

test.afterEach(async () => {
  console.log('after each');
});

test('actual test1', async () => {
  console.log('actual test1');
});

test('actual test2', async () => {
  console.log('actual test2');
});

test.afterAll(async () => {
  console.log('called after all');
});
```

### 🔹 Execution Order
```
beforeAll
  beforeEach → test1 → afterEach
  beforeEach → test2 → afterEach
afterAll
```

### ✅ Use When
- DB setup / token generation
- Browser or environment initialization

---

## 2️⃣ Hooks Inside test.describe (Scoped Hooks)

### 🔹 Code Example
```ts
test.describe('group 1', () => {

  test.beforeAll(async () => {
    console.log('group before all');
  });

  test.beforeEach(async () => {
    console.log('group before each');
  });

  test('test 1', async () => {
    console.log('test 1');
  });

  test('test 2', async () => {
    console.log('test 2');
  });

  test.afterAll(async () => {
    console.log('group after all');
  });
});
```

### 🔹 Key Point
- Hooks apply **only to tests inside describe**
- Clean & modular test design

---

## 3️⃣ Serial Execution (test.describe.serial)

### 🔹 Code Example
```ts
test.describe.serial('serial group', () => {

  test('test 1', async () => {
    console.log('test 1');
  });

  test('test 2 - fail', async () => {
    console.log('test 2');
    expect(true).toBe(false);
  });

  test('test 3', async () => {
    console.log('test 3');
  });
});
```

### 🔹 Behavior
- If **any test fails**, remaining tests are **skipped**
- If retry is their for e.g 2 time retries for fail test in that case if 2nd test fail then 1st and 2nd both run again.

### ✅ Use When
- Dependent workflows (checkout, multi-step flows)

---

## 4️⃣ UI Tests Using Hooks with Page Fixture

### 🔹 Code Example
```ts
test.describe('login page tests', () => {

  test.beforeEach(async ({ page }) => {
    await page.goto('https://naveenautomationlabs.com/opencart/index.php?route=account/login');
  });

  test('title validation', async ({ page }) => {
    await expect(page).toHaveTitle('Account Login');
  });

  test('header validation', async ({ page }) => {
    await expect(page.getByText('Returning Customer', { exact: true })).toBeVisible();
  });

  test.afterEach(async ({ page }) => {
    await page.close();
  });
});
```

### ✅ Best Practice
- Put **navigation in beforeEach**
- Keep tests clean and readable

---



## 8️⃣ Interview-Ready One-Liners

- **beforeAll** runs once per file
- **beforeEach** runs before every test
- **serial** stops execution on failure

---

## Import differnt types 

### 1️⃣ Named Imports ( `{ }` REQUIRED )

### 📌 When to use
- Module exports **multiple named members**
- You want **specific utilities** from a module

### 🔹 Example: Playwright
```ts
import { test, expect } from '@playwright/test';
```

### 🔹 Internal Export (conceptual)
```ts
export const test = ...
export const expect = ...
export const chromium = ...
```

### 🔹 Rules
- `{ }` is **MANDATORY**
- Names are **FIXED** (must match export)
- You may rename using `as`

```ts
import { test as pwTest, expect as pwExpect } from '@playwright/test';
```

### ❌ Invalid
```ts
import test from '@playwright/test';
```

### 🧠 Key Takeaway
> Named exports → `{}` required → names are fixed unless aliased

---

### 2️⃣ Default Import ( `{ }` NOT used )

### 📌 When to use
- Module exports **one main value**
- Common in Node.js & JSON

### 🔹 Example: fs
```ts
import fs from 'fs';
```

### 🔹 Example: JSON
```ts
import registrationData from '../regdata.json' assert { type: 'json' };
```

### 🔹 Rules
- `{ }` ❌ NOT used
- Import name is **COMPLETELY FLEXIBLE**
- Only **ONE default export** exists

```ts
import abc from 'fs';
import fileSystem from 'fs';
import data from '../regdata.json';
```

All are ✅ valid.

### ❌ Invalid
```ts
import { fs } from 'fs';
import { data } from '../regdata.json';
```

### 🧠 Key Takeaway
> Default export → no `{}` → name can be anything

---

### 3️⃣ Mixed Import ( Default + Named )

### 📌 When to use
- Module exports **one default AND multiple named exports**

### 🔹 Export Example
```ts
export default function logger() {}
export const info = () => {}
export const error = () => {}
```

### 🔹 Import Syntax
```ts
import logger, { info, error } from './logger';
```

### 🔹 Rules
- Default import name → flexible
- Named imports → fixed (or aliased)

```ts
import myLogger, { info as logInfo } from './logger';
```

### 🧠 Key Takeaway
> Default + named → both syntaxes together

---

### 4️⃣ Name Flexibility Summary (VERY IMPORTANT)

| Import Type | `{ }` | Name Flexible? | Example |
|-----------|------|---------------|--------|
| Named | ✅ Required | ❌ Fixed | `{ test, expect }` |
| Default | ❌ Not used | ✅ Any name | `fs`, `abc`, `data` |
| Mixed | Both | Default ✅ / Named ❌ | `logger, { info }` |

---

### 5️⃣ Real Playwright Examples (Correct Usage)

### ✅ Playwright
```ts
import { test, expect } from '@playwright/test';
```

### ✅ JSON Data Provider (BEST)
```ts
import users from '../regdata.json' assert { type: 'json' };
```

### ✅ fs (if really needed)
```ts
import fs from 'fs';
fs.readFileSync(...);
```

---

### 6️⃣ Common Interview Traps ❌

```ts
import fs from '@playwright/test';   // ❌
import { test } from 'fs';            // ❌
import { data } from './data.json';   // ❌
```

---

### 7️⃣ One-Line Interview Answers

- **Named exports** require `{ }`
- **Default exports** never use `{ }`
- **JSON always has default export**
- **Alias does not change API**

---

## ✅ Final Golden Rule (Pin This)

> 👉Curly braces are required only for named exports.**
> 👉 Default exports allow any variable name.**
> 👉 Use **describe + hooks + JSON import** for enterprise-grade Playwright frameworks.
> 👉 This structure is **scalable, readable, fast, and interview-safe**.

-----