...
- innerText() vs textContent() – To know whether you are validating only visible UI text or including hidden DOM text.
- check() / uncheck() vs click() – To control checkbox/radio state safely instead of blindly toggling it.
- Roles (header, textbox, img, radio, checkbox) – To locate elements semantically using accessibility, making tests stable and user-centric.
- page.$() and page.$$() – To understand low-level DOM access and why it is avoided in favor of stable locators.
- heading role (h1–h6) – To correctly validate page structure using semantic heading levels.


## 1️⃣ `innerText()` vs `textContent()` (WITHOUT first()/last())
...
### Output
```text
textContent() → Hidden HeaderVisible Header
innerText()   → Visible Header
```
...
✅ **Answer:** Hidden text **is printed by `textContent()`**, not by `innerText()`.
...
```text
innerText()   → Visible Header
textContent() → Visible Header
```

### Difference
| Method | Reads | Visibility aware | Use case |
|---|---|---|---|
| `innerText()` | Visible text only | ✅ Yes | UI validation |
| `textContent()` | All text (raw DOM) | ❌ No | Debug / data read |

✅ **Best practice:** Use `innerText()` for assertions.

---

## 2️⃣ `check()` / `uncheck()` vs `click()`
...
|---|---|---|
| `check()` | Ensures checkbox ON | ✅ Yes |
| `uncheck()` | Ensures checkbox OFF | ✅ Yes |
| `click()` | Toggles state blindly | ❌ No |
...
✅ **Best practice:** Use `check()` / `uncheck()` for checkbox & radio.

---

## 3️⃣ Roles with Examples (header, textbox, img, radio, checkbox)
...
## 4️⃣ `page.$()` and `page.$$()` (LOW-LEVEL API – WITH PRACTICAL EXAMPLES)
...
⚠️ Risk: No auto-wait, fails if element not ready

---

### `page.$$()` – multiple elements
```ts
...
✔ Auto-wait  ✔ Retry  ✔ Stable

---
 `page.$()` and `page.$$()` (LOW‑LEVEL API)
...
❌ Avoid in modern Playwright tests

---

## 5️⃣ Locator vs `$ / $$` (COMPARISON)
...
## 6️⃣ Best Practices (FINAL & MANDATORY)
...
### 2️⃣ `textContent()` vs `innerText()` (Truth recap)

- `textContent()` → returns **visible + hidden text** inside the tag
- `innerText()` → returns **only visible text**

---

### 3️⃣ Tags where `getByRole()` is NOT eligible

❌ `getByRole()` **does NOT work directly** on:
- `<span>`
- `<div>`
- `<p>`

👉 These tags have **no implicit ARIA role**.
...
### 4️⃣ How `getByRole()` finds elements (VERY IMPORTANT)

`getByRole()` works based on **accessible name**, which can come from:

1️⃣ `<label>` tag text  
2️⃣ `aria-label` attribute  
3️⃣ `placeholder` attribute  
4️⃣ Visible text (for buttons, links, headings)
