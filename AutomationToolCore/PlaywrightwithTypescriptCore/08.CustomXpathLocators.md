...
- Learn XPath so you’re never blocked—use it only when better locators don’t exist.
...
### Use case
❌ **Do NOT use** (breaks on any UI change)

---

## 2️⃣ Relative XPath (Custom XPath)
...
### Use case
⚠️ Avoid unless tag is unknown

---

## 6️⃣ Text Matching using `text()`
...
## 9️⃣ `contains()` with Text + Attribute
...
### Please note Ends-with() not exist in xpath.
...
### Use case
⚠️ Use only when order is guaranteed
...
- ⚠️ Avoid indexing XPath when possible
- ✅ Prefer relative XPath with stable attributes
- ⭐ Prefer `getByTestId()` / `getByRole()` over XPath

---

## Architect Golden Rule 🏆

> **XPath is powerful, but power comes with fragility—use it only as a last resort.**
