...
![alt text](images/selenium-webdriver-architecture.png)
...
- WebSocket communicate by RPC(Remote Procedure Call) call in JSON format.
- Faster and more stable execution
- Direct control over browser engines
- Better handling of modern web apps (React, Angular, Vue)

![alt text](images/Playwright_Arch.png)

---

## HTTP vs WebSocket (Core Difference)
...
![alt text](images/http-vs-websocket.png)

![alt text](images/Sel-vs-Pw.png)
...
##  How Playwright Talks to Different Browsers
...
### Example Mac: Brave with executablePath
...
### Exmple Windows : Opera

```ts
executablePath: 'C:\\Users\\Shree Vi\\AppData\\Local\\Programs\\Opera\\opera.exe'
```
...
| Browser  | Method         | Reason                      | Orignal Browser |
| -------- | -------------- | --------------------------- | --------------- |
| Chromium | Built-in       | Playwright-managed Chromium | No              |
| Chrome   | channel        | Official Chromium support   | Yes             |
| Edge     | channel        | Official Chromium support   | Yes             |
| Brave    | executablePath | Third-party Chromium        | Yes             |
| Opera    | executablePath | Third-party Chromium        | Yes             |
| Firefox  | Built-in       | Custom patched Nightly      | No              |
| Safari   | Built-in       | Custom WebKit build         | No              |
...
## 13. Playwright vs Selenium WebDriver – Feature Comparison


| #   | Feature / Aspect      | Playwright                                                     | Selenium WebDriver                                  |
| --- | --------------------- | -------------------------------------------------------------- | --------------------------------------------------- |
| 1   | Architecture          | Direct communication with browsers via CDP / custom protocols  | Uses W3C WebDriver protocol via browser drivers     |
| 2   | Speed                 | Faster (no extra HTTP hops, WebSocket-based)                   | Slower due to multiple layers of HTTP communication |
| 3   | Browser Contexts      | Built-in browser contexts → easy multi-session / user testing  | Needs separate drivers or profiles                  |
| 4   | Auto-waiting          | Automatic waits for elements, network, and animations          | Needs explicit waits (WebDriverWait)                |
| 5   | Cross-browser Support | Chromium, Firefox, WebKit (Safari engine), Chrome, Edge, Brave | Chromium, Firefox, Safari, Edge, IE, Opera, Brave   |
| 6   | Cross-platform        | macOS, Windows, Linux                                          | macOS, Windows, Linux                               |
| 7   | Mobile Testing        | Can emulate mobile devices via WebKit & Chromium               | Limited mobile support                              |
| 8   | Parallel Testing      | Built-in via Playwright Test runner                            | Needs Selenium Grid or third-party setup            |
| 9   | Network Interception  | Direct API to intercept, mock & modify requests                | No native support (needs CDP + bidi)                |
| 10  | Test Isolation        | Native browser contexts → no session sharing                   | Needs workarounds                                   |
| 11  | Setup Complexity      | Easy, installs browsers automatically                          | Easier now with Selenium Manager / WebDriverManager |
| 12  | Stability             | More stable, fewer flaky tests                                 | Comparatively flaky due to async timing issues      |
| 13  | Modern Web Apps (SPA) | Excellent support (React, Angular, Vue)                        | Complex handling for modern SPAs                    |
| 14  | API Testing           | Yes (built-in API testing support)                             | No (needs external libraries)                       |
| 15  | Screenshots           | Yes                                                            | Yes                                                 |
| 16  | Video Recording       | Yes                                                            | No (needs third-party tools)                        |
| 17  | Modes                 | UI, Codegen, Inspector, Trace Viewer (time travel)             | No native equivalents                               |
| 18  | Reports               | Built-in rich HTML reports                                     | No (requires third-party reporting tools)           |
