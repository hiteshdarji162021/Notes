...
## 2️⃣ SVG Element Inside Iframe (Map / Graph)
...
### Best Practices
- Tooltips may NOT exist → validate **color or legend** instead
- Use `boundingBox()` for precise interaction
- Prefer API + UI validation for maps

---

## 3️⃣ Shadow DOM Element (Normal Page)
...
- Inside frame → all locators allowed
- `nth()` / `first()` acceptable if frames are identical

---

## 5️⃣ Pseudo Elements (`::before / ::after`)
...
👉 **Playwright cannot locate them directly** → we must read **computed styles**.

---

## 2️⃣ Approach 1 – Using `locator.evaluate()` (Recommended)
...
## 3️⃣ Approach 2 – Using `page.evaluate()` (Alternative)
...
⚠️ **Avoid for large test suites** (less maintainable)

---

## 4️⃣ Reading CSS Properties from Pseudo Elements

### Example – Color of `::before`

#### Approach 1 – Locator Based
```ts
...
## 5️⃣ Validating Normal Element Styles (No Pseudo Elements)

### Example – Button Color Validation
...
## 6️⃣ Handling Alerts Using Page Context
...
## 7️⃣ Best Practices (Interview + Real Project)
...
> “Pseudo elements don’t exist in the DOM, so in Playwright we validate them using `getComputedStyle` via `evaluate()` instead of locators.”

---

## 9️⃣ Comparison Table – All Approaches
...
| Safe for large suites | ✅ Yes | ⚠️ Limited |
| Best for pseudo-elements | ✅ Recommended | ⚠️ Alternative |
| Uses Playwright Locator | ✅ Yes | ❌ No |
| Risk of null selector | ❌ Very low | ⚠️ High |

---

## 🔟 Best Practices (Production + Interview)
...
## 1️⃣1️⃣ When This Skill Matters Most
...
## 1️⃣2️⃣ Interview Power Line

> “Pseudo elements are not part of the DOM, so in Playwright we validate them using `getComputedStyle` via `locator.evaluate()` instead of locators or XPath.”


---

## 🏆 Global Best Practices (SAVE THIS)

- SVG → Use XPath `name()` and `local-name()` only when required
- Prefer HTML wrappers around SVG (ask devs)
- `frameLocator()` → CSS only for iframe
- Shadow DOM → Playwright handles automatically
- Use `evaluate()` as **escape hatch**, not default
- If UI data not visible → validate API or CSS state
